...
class Parser:
    ...
    def __init__(self, input_name):
        ...
        self.commands_table = {"add" : C_ARITHMETIC, "sub" : C_ARITHMETIC, "neg" : C_ARITHMETIC, "eq" : C_ARITHMETIC,
                                ...
                                "not" : C_ARITHMETIC, "pop" : C_POP, "push" : C_PUSH, "label" : C_LABEL, "if-goto" : C_IF,
    ...
    def arg1(self):
        if self.curr_type == C_ARITHMETIC:
            return self.next_line
        if (self.curr_type == C_POP) or (self.curr_type == C_PUSH) or (self.curr_type == C_CALL) or (self.curr_type == C_FUNCTION):
            sep = self.next_line.index(" ")
            temp_line = self.next_line[sep + 1:]
            sep = temp_line.index(" ")
            return temp_line[:sep]
        if (self.curr_type == C_IF) or (self.curr_type == C_GOTO) or (self.curr_type == C_LABEL):
            sep = self.next_line.index(" ")
            return self.next_line[sep + 1:]
        return "null"
